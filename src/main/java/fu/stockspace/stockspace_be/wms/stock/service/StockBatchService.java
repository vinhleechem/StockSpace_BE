...
public class StockBatchService {
    ...
    public PagedResponse<WarehouseStockOverviewResponse> getStockOverviewByWarehouse(
        ...
        return PagedResponse.fromPage(page, row -> {
            ...
            return WarehouseStockOverviewResponse.builder()
                    ...
                    .warehouseId(warehouse.getId())
                    .warehouseName(warehouse.getName())
                    .totalQuantity(masked ? null : row.getTotalQuantity())
                    .reservedQuantity(masked ? null : reserved)
                    .availableQuantity(masked ? null : Math.max(0L, row.getTotalQuantity() - reserved))
    ...
    public StockSummaryResponse getStockSummaryBySku(UUID tenantId, UUID skuId, UUID staffId) {
        ...
        List<StockLocationDto> locations = batches.stream()
                .map(b -> {
                    ...
                    return StockLocationDto.builder()
                        ...
                        .quantity(masked ? null : b.getQuantity())
                        .reservedQuantity(masked ? null : reservedForBatch)
                        .availableQuantity(masked ? null : Math.max(0, b.getQuantity() - reservedForBatch))
                        .quantityMasked(masked)
                        .build();
                })
        ...
        return StockSummaryResponse.builder()
                ...
                .totalQuantity(quantityMasked ? null : totalQuantity)
                .reservedQuantity(quantityMasked ? null : reservedQuantity)
                .availableQuantity(quantityMasked ? null : Math.max(0, totalQuantity - reservedQuantity))
    ...
    private StockBatchResponse mapToResponse(StockBatch b, List<InventoryAudit> audits) {
        ...
        return StockBatchResponse.builder()
                ...
                .binName(b.getBin() != null ? b.getBin().getName() : null)
                .quantity(masked ? null : b.getQuantity())
                .reservedQuantity(masked ? null : reservedQuantity)
                .availableQuantity(masked ? null : Math.max(0, b.getQuantity() - reservedQuantity))
                .quantityMasked(masked)
